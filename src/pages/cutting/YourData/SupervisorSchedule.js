...
function Supervisor(props) {
  ...
  const copy = async () => {
    try {
      const resp = await getCuttingSupervisorCopy();
      console.log(resp);
      setMsg(resp.msg);
      setOpen(true);
      loadData();
  ...
  const addSupervisor = async (data) => {
    try {
      const resp = await addCuttingSupervisorSingle(data);
      // console.log(resp);
      setMsg("Added");
      setOpen(true);
      loadData();
  ...
  const updateSupervisor = async () => {
    try {
      const resp = await updateCuttingSupervisorSingle(userdata);
      console.log(resp);
